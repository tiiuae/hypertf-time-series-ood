...
for ds in list(data_info.UCR_DATASETS_TYPE.keys()):
    dataset_to_cfg[ds] = "configs/auxiliary_oe/ucr_aux_contrastive.yaml"
for ds in list(data_info.UEA_DATASETS_TYPE.keys()):
    dataset_to_cfg[ds] = "configs/auxiliary_oe/uea_aux_contrastive.yaml"

# Use nested exp_name structure
# Final experiments will be tuples of (exp_name, *params_to_chk, seed)
# (exp_name, same_outlier_exposure_sub_type, oe_sampling_feat_strategy)
EXPERIMENTS = [
    ("main_table/hypertf_aux_outlier_near_aux", True, "pre_match_features_to_id"),
    ("main_table/hypertf_aux_outlier_far_aux", False, "pre_match_features_to_id"),
]


def build_cmd(gpu_id, exp_name, same_outlier_exposure_sub_type, oe_sampling_feat_strategy, seed, dataset):
    """
